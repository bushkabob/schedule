...
const SelectOptionScreen = () => {
    ...
    useLayoutEffect(() => {
        navigation.setOptions({ 
            ...
            headerRight: () => (
                <Button title="Save" onPress={() => (navigation.navigate("AddAssignment", { selectedData: { category: route.params.name, selectedString: selected} })) } />
            )
        })
    ...
    const renderRow = (item: string, isSelected: boolean) => {
        return <Cell title={item} onPress={()=>(setSelected(item))} accessory={isSelected?"Checkmark":undefined} />
    }
